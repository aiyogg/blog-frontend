        ...
        function _storageProvider(storageType) {
            ...
            var _storage = function () {
                ...
                _store = {
                    ...
                    //以$storage数据为基础进行双向同步($storage内部<->web本地storage)
                    // $sync: function () {
                    //     console.log('$sync');
                    //     var temp$storage;
                    //     if (!equals(_store, _last$storage)) {
                    //         temp$storage = copy(_last$storage);
                    //         forEach(_store, function (v, k) {
                    //             if (isDefined(v) && '$' !== k[0]) {
                    //                 _webStorage.setItem(storageKeyPrefix + k, serializer(v));
